...
class ShiftAnalyticsService {
   ...
   * Сохранение в Redis
   */
  async saveToRedis(shiftData, loadData) {
    const {
      departmentObjectId,
      accountId,
      assigneeEmail
    ...
    // Ключ: Department:{departmentObjectId}:{accountId}:{email}
    const key = `Department:${departmentObjectId}:${accountId}:${assigneeEmail}`;

    // Объединяем данные смены и нагрузки
    const redisData = {
      ...shiftData,
      analytics: loadData,
    ...
    try {
      ...
      // Устанавливаем TTL из shiftData (если есть)
      if (shiftData.ttl) {
        await redisClient.expire(key, shiftData.ttl);
      }

      console.log(`[Analytics] 💾 Сохранено в Redis: ${key}`);

      // Добавляем в индекс департамента
